#!/bin/bash
# Run as root: bash ops/deploy-registry.sh FULL_COMMIT_SHA
set -Eeuo pipefail
umask 077
...
test "$code" = 200
trap - ERR
echo 'ГОТОВО: реестр людей и участков установлен. Главная HTTP 200, админка HTTP 302.'
