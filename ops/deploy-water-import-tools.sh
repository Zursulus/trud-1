#!/bin/bash
# Import-tools-only production release.
# Run as root: bash deploy-water-import-tools.sh TARGET_SHA EXPECTED_PRODUCTION_SHA
set -Eeuo pipefail
umask 077
...
ALLOWED='^(backend/water/management/commands/import_water_readings\.py|backend/water/management/commands/import_water_undated_notes\.py|backend/water/package_imports\.py|backend/water/test_import_water_readings\.py|backend/water/test_package_address_uniqueness\.py|backend/water/test_package_unknown_dates\.py|backend/water/test_import_water_undated_notes\.py|ops/deploy-water-import-tools\.sh)$'
...
BACKUP=$(mktemp -d /var/backups/trud-water-import-tools.XXXXXXXX)
...
echo 'Схема БД не изменялась; команды импорта только добавлены в код.'
