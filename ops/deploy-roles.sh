#!/bin/bash
# Run as root: bash ops/deploy-roles.sh FULL_COMMIT_SHA
set -Eeuo pipefail
umask 077
...
if [ "$PREVIOUS" != 752b72a1bd4b4d2c82e3c65f054ef8d0a20b3abd ] && [ "$PREVIOUS" != "$TARGET" ]; then
    echo 'На сервере другая версия. Остановка для проверки совместимости.'; exit 1
fi
BACKUP=$(mktemp -d /var/backups/trud-roles.XXXXXXXX)
...
rollback() {
    ...
    echo "Копия и сведения об установке: $BACKUP"
    exit "$result"
}
trap rollback ERR
# Additive permission metadata only: compatible with the previous code.
CHANGED=1
...
test "$code" = 200
trap - ERR
echo 'ГОТОВО: роли и журнал установлены. Главная HTTP 200, админка HTTP 302.'
echo 'Существующие пользователи не изменены. Назначьте сотрудникам одну штатную роль.'
