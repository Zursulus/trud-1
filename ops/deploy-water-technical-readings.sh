#!/bin/bash
# Isolated release for technical historical water-reading import.
# Run as root: bash deploy-water-technical-readings.sh TARGET_SHA EXPECTED_PRODUCTION_SHA
set -Eeuo pipefail
umask 077
...
ALLOWED='^(backend/water/management/commands/import_water_technical_readings\.py|backend/water/test_import_water_technical_readings\.py|ops/deploy-water-technical-readings\.sh)$'
...
BACKUP=$(mktemp -d /var/backups/trud-water-technical-readings.XXXXXXXX)
...
echo 'Схема БД не изменялась; добавлена только изолированная команда импорта технических показаний.'
